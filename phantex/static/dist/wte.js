...
var networkState = /* @__PURE__ */ new Map();
/**
...
function render(data) {
	...
	for (const record of networks) {
		if (!networkState.has(record.bssid)) appendLog(`new network: ${record.ssid && record.ssid !== "[hidden]" ? `${record.ssid} (${record.bssid})` : record.bssid} ch=${record.channel ?? "--"} signal=${record.signal ?? "--"} ${record.security ?? "open"}`, "log-new");
		if (!historyState.has(record.bssid)) {
			historyState.add(record.bssid);
			updateSeenCount();
		}
	}
	for (const record of networks) networkState.set(record.bssid, {
		record,
		lastUpdated: now
	});
	for (const [bssid, entry] of networkState.entries()) if (!incomingBssids.has(bssid)) {
		const wasRecent = now - entry.lastUpdated <= STALE_THRESHOLD_MS;
		const isNowStale = now - entry.lastUpdated > STALE_THRESHOLD_MS;
		if (wasRecent && isNowStale) appendLog(`network stale: ${entry.record.ssid && entry.record.ssid !== "[hidden]" ? `${entry.record.ssid} (${bssid})` : bssid}`, "log-stale");
	}
	if (networks.length > 0) appendLog(`scan complete: ${networks.length} network(s) visible`);
	else appendLog("scan complete: no networks found");
	updateTable(now);
}
/**
...
function updateTable(now) {
	...
	for (const row of tbody.querySelectorAll("tr[data-bssid]")) existingRows.set(row.dataset.bssid, row);
...
async function loadHistory() {
	...
	try {
		...
		if (!res.ok) throw new Error(`HTTP ${res.status}`);
		_historyRows = (await res.json()).networks || [];
